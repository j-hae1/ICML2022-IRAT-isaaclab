...
from irat_code.algorithms.utils.distributions import FixedCategorical, FixedNormal, FixedBernoulli
# torch.autograd.set_detect_anomaly(True)


class RMappoTrSynSeriesImprovement:
    """
    ...
    def __init__(self,
        ...
        self.ikl_coef = self.idv_kl_coef
        self.idv_kl_coef_use_exp = args.idv_kl_coef_use_exp
        ...
        self.tkl_coef = self.team_kl_coef
        self.team_kl_coef_use_exp = args.team_kl_coef_use_exp
        ...
        self.idv_clip_flag = args.idv_clip_flag
        self.idv_clip_use_min = args.idv_clip_use_min
        self.idv_clip_flag_refine = args.idv_clip_flag_refine
        self.idv_clip_use_time = args.idv_clip_use_time
        self.idv_kl_use_time = args.idv_kl_use_time
        self.idv_clip_update_refine = args.idv_clip_update_refine
    ...
    def ppo_update(self, sample, episode, update_actor=True):
        ...
        idv_action_log_probs_batch_clone = idv_action_log_probs_batch.clone().detach()
        ...
        idv_action_log_probs_batch_clone[idv_action_log_probs_batch_clone < -50.] = -50.
        team_action_log_probs_batch[team_action_log_probs_batch < -50.] = -50.
        ...
        idv_kl_loss = 0
        idv_cross_entropy = torch.zeros(1).to(**self.tpdv)
        team_entropy = 0
        ...
        team_kl_loss = 0
        team_cross_entropy = torch.zeros(1).to(**self.tpdv)
        idv_entropy = 0
        ...
        team_max_actions_log_probs = []
        idv_max_actions_log_probs = []
        # print(team_new_act_dists)
        # print(team_act_dists_batch.shape)
        assert len(team_new_act_dists) == 1
        for ai in range(len(team_new_act_dists)):
            if type(idv_new_act_dists[ai]) == FixedCategorical:
                if self.idv_clip_use_present:
                    ...
                    tmp_probs = torch.stack(tmp_probs).to(**self.tpdv)
                other_dists = type(idv_new_act_dists[ai])(probs=tmp_probs)
                idv_other_dists = type(idv_new_act_dists[ai])(probs=idv_new_act_dists[ai].probs.clone().detach())
                team_dists_std = torch.std(other_dists.probs.clone().detach(), dim=-1, keepdim=True)
                idv_dists_std = torch.std(idv_other_dists.probs.clone().detach(), dim=-1, keepdim=True)
                # print(other_dists.mode())
                # print(other_dists.log_probs(other_dists.mode()))
            elif type(idv_new_act_dists[ai]) == FixedNormal:
                if self.idv_clip_use_present:
                    tmp_mu = team_new_act_dists[ai].loc.clone().detach()
                    tmp_sigma = team_new_act_dists[ai].scale.clone().detach()
                    ...
                    tmp_sigma = torch.stack(tmp_sigma).to(**self.tpdv)
                other_dists = type(idv_new_act_dists[ai])(loc=tmp_mu, scale=tmp_sigma)
                idv_other_dists = type(idv_new_act_dists[ai])(loc=idv_new_act_dists[ai].loc.clone().detach(),
                                                              scale=idv_new_act_dists[ai].scale.clone().detach())
                team_dists_std = torch.prod(other_dists.scale.clone().detach(), dim=-1, keepdim=True)
                idv_dists_std = torch.prod(idv_other_dists.scale.clone().detach(), dim=-1, keepdim=True)
                # print(ai, other_dists)
            else:
                raise NotImplementedError
            team_tmp_max = other_dists.log_probs(other_dists.mode())
            team_max_actions_log_probs.append(team_tmp_max)

            # idv_other_dists = copy.deepcopy(idv_new_act_dists[ai])
            idv_tmp_max = idv_other_dists.log_probs(idv_other_dists.mode()).clone().detach()
            idv_max_actions_log_probs.append(idv_tmp_max)

        team_max_actions_log_probs = team_max_actions_log_probs[0].detach().to(**self.tpdv)
        idv_max_actions_log_probs = idv_max_actions_log_probs[0].detach().to(**self.tpdv)

        assert team_max_actions_log_probs.shape == idv_max_actions_log_probs.shape
        team_idv_ratio = (torch.exp(team_max_actions_log_probs) + 0.001) / (torch.exp(idv_max_actions_log_probs) + 0.001)

        assert team_dists_std.shape == idv_dists_std.shape
        team_idv_std_ratio = team_dists_std / idv_dists_std
        ...
        if self.idv_clip_flag_refine == 0:
            assert team_idv_ratio.shape == so_weights.shape
            idv_flag = so_weights.clone().detach() * team_idv_ratio.clone().detach()
            if self.idv_clip_update_refine:
                so_weights = so_weights * team_idv_ratio.clone().detach()
        elif self.idv_clip_flag_refine == 1:
            assert team_idv_std_ratio.shape == so_weights.shape, str(team_idv_std_ratio.shape) + ", " + str(so_weights.shape)
            idv_flag = so_weights.clone().detach() * team_idv_std_ratio.clone().detach()
            if self.idv_clip_update_refine:
                so_weights = so_weights * team_idv_std_ratio.clone().detach()
        else:
            idv_flag = so_weights.clone().detach()
        idv_flag = idv_flag.to(**self.tpdv)

        if self.idv_clip_use_min:
            clp = torch.clamp(so_weights, 0., self.idv_clip_ratio)
        else:
            clp = torch.clamp(so_weights, 1.0 - self.idv_clip_ratio, 1.0 + self.idv_clip_ratio)
        surr3 = clp * idv_adv_targ

        dy = dict(dtype=bool, device=self.device)
        if self.idv_clip_flag == 0:         # min
            tc_flag_min = torch.ones_like(so_weights).to(**dy)
            tc_flag_max = torch.zeros_like(so_weights).to(**dy)
            tc_flag_org = torch.zeros_like(so_weights).to(**dy)
        elif self.idv_clip_flag == 1:       # max
            tc_flag_min = torch.zeros_like(so_weights).to(**dy)
            tc_flag_max = torch.ones_like(so_weights).to(**dy)
            tc_flag_org = torch.zeros_like(so_weights).to(**dy)
        elif self.idv_clip_flag == 2:       # pi_idv/pi_team 或 (pi_idv/pi_team) * (pi_team_max/pi_idv_max) - idv_flag作为判定条件
            tc_flag_min = idv_flag >= 1.0
            tc_flag_max = ~tc_flag_min
            tc_flag_org = torch.zeros_like(so_weights).to(**dy)
        elif self.idv_clip_flag == 3:       # A_team作为判定条件
            tc_flag_min = team_adv_targ <= 0.
            tc_flag_max = ~tc_flag_min
            tc_flag_org = torch.zeros_like(so_weights).to(**dy)
        elif self.idv_clip_flag == 4:       # A_team和idv_flag共同作为判定条件，条件外的按照原本的mappo更新
            tc_flag_min = (team_adv_targ <= 0.) & (idv_flag >= 1.0)
            tc_flag_max = (team_adv_targ > 0.) & (idv_flag < 1.0)
            tc_flag_org = ~(tc_flag_min | tc_flag_max)
        elif self.idv_clip_flag == 5:       # A_team和idv_flag共同作为判定条件，条件外更多限制更新
            tc_flag_min = ((team_adv_targ <= 0.) & (idv_adv_targ > 0.)) | \
                          ((idv_flag >= 1.0) & (((team_adv_targ > 0.) & (idv_adv_targ > 0.))
                                                | ((team_adv_targ <= 0.) & (idv_adv_targ <= 0.))))
            tc_flag_max = ((team_adv_targ > 0.) & (idv_adv_targ <= 0.)) | \
                          ((idv_flag < 1.0) & (((team_adv_targ > 0.) & (idv_adv_targ > 0.))
                                                | ((team_adv_targ <= 0.) & (idv_adv_targ <= 0.))))
            assert (tc_flag_max == ~tc_flag_min).all()
            tc_flag_org = torch.zeros_like(so_weights).to(**dy)
        elif self.idv_clip_flag == 6:       # A_team和idv_flag共同作为判定条件，条件外更多限制更新
            tc_flag_min = ((team_adv_targ <= 0.) & (idv_adv_targ <= 0.)) | \
                          ((idv_flag >= 1.0) & (((team_adv_targ > 0.) & (idv_adv_targ <= 0.))
                                                | ((team_adv_targ <= 0.) & (idv_adv_targ > 0.))))
            tc_flag_max = ~tc_flag_min
        ...
        tc_flag_min = tc_flag_min.detach()
        tc_flag_max = tc_flag_max.detach()
        tc_flag_org = tc_flag_org.detach()
        ...
        ts12 = surr1 <= surr2
        ts13 = surr1 <= surr3
        ts1_flag = ts12 & ((tc_flag_min & ts13) | (tc_flag_max & ~ts13) | tc_flag_org)
        ts1 = ts1_flag.float().sum() / tn
        ts1_min = (ts12 & (tc_flag_min & ts13)).float().sum() / tn
        ts1_max = (ts12 & (tc_flag_max & ~ts13)).float().sum() / tn
        ts1_org = (ts12 & tc_flag_org).float().sum() / tn
        tsl1 = surr1.clone().detach()[ts1_flag].mean()

        idv_min = torch.min(surr1, surr2)
        ts31 = surr3 <= idv_min
        t3_flag = tgs & tls & ((tc_flag_min & ts31) | (tc_flag_max & ~ts31))
        ts3 = t3_flag.float().sum() / tn
        ts3_min = (tgs & tls & (tc_flag_min & ts31)).float().sum() / tn
        ts3_max = (tgs & tls & (tc_flag_max & ~ts31)).float().sum() / tn
        tsl3 = surr3.clone().detach()[t3_flag].mean()

        if self.idv_use_two_clip and episode >= self.idv_clip_use_time:
            tc_min = torch.min(idv_min, surr3)
            tc_max = torch.max(idv_min, surr3)
            idv_min = tc_flag_min.float() * tc_min + tc_flag_max.float() * tc_max + tc_flag_org.float() * idv_min
        ...
        if self.idv_kl_coef_use_exp:
            self.ikl_coef = np.exp(self.idv_kl_coef)
        else:
            self.ikl_coef = self.idv_kl_coef
        if self.idv_use_kl_loss and episode >= self.idv_kl_use_time:
            idv_loss += self.ikl_coef * idv_kl_loss
            idv_kl_abs = self.ikl_coef * idv_kl_loss.clone().detach()
        elif self.idv_use_cross_entropy:
            idv_loss += self.ikl_coef * idv_cross_entropy
            idv_kl_abs = self.ikl_coef * idv_cross_entropy.clone().detach()
        else:
            idv_kl_abs = torch.zeros(1).to(**self.tpdv)
        ...
        if self.change_reward and episode > self.change_reward_episode:
            if self.change_use_policy == "team":
                idv_loss = torch.zeros(1, requires_grad=True).to(**self.tpdv)
                idv_value_loss = torch.zeros(1, requires_grad=True).to(**self.tpdv)
                idv_ppo_abs = torch.zeros(1).to(**self.tpdv)
                idv_entropy_abs = torch.zeros(1).to(**self.tpdv)
                idv_kl_abs = torch.zeros(1).to(**self.tpdv)
            else:
                idv_min = torch.min(surr1, surr2)
                if self._use_policy_active_masks:
        ...
        if self.team_clip_use_present:
            ...
            team_imp_weights = torch.exp(team_new_action_log_probs - idv_action_log_probs_batch_clone)
        ...
        if self.team_use_entropy:
            ...
            team_loss = team_policy_loss
            team_entropy_abs = torch.zeros(1).to(**self.tpdv)

        if self.team_kl_coef_use_exp:
            self.tkl_coef = np.exp(1./(self.team_kl_coef - 1e-5))
        else:
            self.tkl_coef = self.team_kl_coef

        if self.team_use_kl_loss and episode >= self.team_kl_use_time:
            team_loss += self.tkl_coef * team_kl_loss
            team_kl_abs = self.tkl_coef * team_kl_loss.clone().detach()
        elif self.team_use_cross_entropy:
            team_loss += self.tkl_coef * team_cross_entropy
            team_kl_abs = self.tkl_coef * team_cross_entropy.clone().detach()
        else:
            team_kl_abs = torch.zeros(1).to(**self.tpdv)
        ...
        if self.change_reward and episode > self.change_reward_episode:
            if self.change_use_policy == "team":
                ...
                team_loss = torch.zeros(1, requires_grad=True).to(**self.tpdv)
                team_value_loss = torch.zeros(1, requires_grad=True).to(**self.tpdv)
                team_ppo_abs = torch.zeros(1).to(**self.tpdv)
                team_entropy_abs = torch.zeros(1).to(**self.tpdv)
                team_kl_abs = torch.zeros(1).to(**self.tpdv)
        else:
            pass
        ...
        return idv_value_loss, team_value_loss, idv_critic_grad_norm, team_critic_grad_norm,\
               ...
               team_loss, team_ppo_abs, team_ppo_prop, team_entropy_prop, team_kl_prop, ts1_min, ts1_max, ts1_org, \
               ts3_min, ts3_max
    ...
    def train(self, buffer, episode, update_actor=True):
        ...
        train_info['Ba_idv_org_min_prop'] = 0
        train_info['Bb_idv_org_max_prop'] = 0
        train_info['Bc_idv_org_org_prop'] = 0
        train_info['Bd_idv_new_min_prop'] = 0
        train_info['Be_idv_new_max_prop'] = 0
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                train_info['Ba_idv_org_min_prop'] += ts1_min
                train_info['Bb_idv_org_max_prop'] += ts1_max
                train_info['Bc_idv_org_org_prop'] += ts1_org
                train_info['Bd_idv_new_min_prop'] += ts3_min
                train_info['Be_idv_new_max_prop'] += ts3_max
        ...
        train_info['Ar_idv_kl_coef'] = self.ikl_coef
        train_info['Av_advantages'] = np.nanmean(idv_advantages)

        train_info['Te_team_epsilon^'] = self.team_clip_ratio
        train_info['Tn_team_kl_coef'] = self.tkl_coef
        train_info['Tr_team_advantages'] = np.nanmean(team_advantages)

        self.update_entropy_coef()
        if self.idv_use_two_clip and episode >= self.idv_clip_use_time:
            self.update_idv_clip_ratio()
        if (self.idv_use_kl_loss or self.idv_use_cross_entropy) and episode >= self.idv_kl_use_time:
            self.update_idv_kl_coef()

        if self.team_use_clip:
            self.update_team_clip_ratio()
        if (self.team_use_kl_loss or self.team_use_cross_entropy) and episode >= self.team_kl_use_time:
