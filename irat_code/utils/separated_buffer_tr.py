...
from irat_code.utils.util import check, get_shape_from_obs_space, get_shape_from_act_space
...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        self.other_actions_dists = np.zeros((self.episode_length, self.n_rollout_threads, act_shape), dtype=object)
        self.action_log_probs = np.zeros((self.episode_length, self.n_rollout_threads, act_shape), dtype=np.float32)
        self.other_action_log_probs = np.zeros((self.episode_length, self.n_rollout_threads, act_shape), dtype=np.float32)
    ...
    def insert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, other_actions_dists, action_log_probs, other_action_log_probs,
        ...
        self.other_actions_dists[self.step] = other_actions_dists.copy()
        self.action_log_probs[self.step] = action_log_probs.copy()
        self.other_action_log_probs[self.step] = other_action_log_probs.copy()
    ...
    def chooseinsert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, other_actions_dists, action_log_probs, other_action_log_probs,
                     value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None):
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        other_actions_dists = self.other_actions_dists.reshape(-1, self.other_actions_dists.shape[-1])
        if self.available_actions is not None:
        ...
        other_action_log_probs = self.other_action_log_probs.reshape(-1, self.other_action_log_probs.shape[-1])
        ...
        for indices in sampler:
            ...
            other_actions_dists_batch = other_actions_dists[indices]
            if self.available_actions is not None:
            ...
            other_act_log_pbs_batch = other_action_log_probs[indices]
            if advantages is None:
                adv_targ = None
            else:
                adv_targ = advantages[indices]

            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, other_actions_dists_batch, \
                  value_preds_batch, return_batch, masks_batch, active_masks_batch, \
                  old_action_log_probs_batch, other_act_log_pbs_batch, adv_targ, available_actions_batch
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, n_rollout_threads, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                other_actions_dists_batch.append(self.other_actions_dists[:, ind])
                if self.available_actions is not None:
                ...
                other_act_log_pb_batch.append(self.other_action_log_probs[:, ind])
            ...
            other_actions_dists_batch = np.stack(other_actions_dists_batch, 1)
            if self.available_actions is not None:
            ...
            other_act_log_pb_batch = np.stack(other_act_log_pb_batch, 1)
            ...
            other_actions_dists_batch = _flatten(T, N, other_actions_dists_batch)
            if self.available_actions is not None:
            ...
            other_act_log_pb_batch = _flatten(T, N, other_act_log_pb_batch)
            adv_targ = _flatten(T, N, adv_targ)

            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, other_actions_dists_batch, \
                  value_preds_batch, return_batch, masks_batch, active_masks_batch, \
                  old_action_log_probs_batch, other_act_log_pb_batch, adv_targ, available_actions_batch
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        other_actions_dists = _cast(self.other_actions_dists)
        action_log_probs = _cast(self.action_log_probs)
        other_action_log_probs = _cast(self.other_action_log_probs)
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                other_actions_dists_batch.append(other_actions_dists[ind:ind + data_chunk_length])
                if self.available_actions is not None:
                ...
                other_act_log_pb_batch.append(other_action_log_probs[ind:ind + data_chunk_length])
            ...
            other_actions_dists_batch = np.stack(other_actions_dists_batch)
            if self.available_actions is not None:
            ...
            other_act_log_pb_batch = np.stack(other_act_log_pb_batch)
            ...
            other_actions_dists_batch = _flatten(L, N, other_actions_dists_batch)
            if self.available_actions is not None:
            ...
            other_act_log_pb_batch = _flatten(L, N, other_act_log_pb_batch)
