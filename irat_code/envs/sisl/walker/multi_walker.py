    ...
    def _reset(self):
        ...
        self.hull = self.world.CreateDynamicBody(
            ...
            fixtures=b2FixtureDef(
                shape=b2PolygonShape(vertices=[(x / SCALE, y / SCALE) for x, y in HULL_POLY]),
                density=5.0,
                friction=0.1,
                categoryBits=0x002,
        ...
        for i in [-1, +1]:
            leg = self.world.CreateDynamicBody(
                ...
                angle=(i * 0.05),
                fixtures=b2FixtureDef(shape=b2PolygonShape(box=(LEG_W / 2, LEG_H / 2)), density=1.0,
            ...
            lower = self.world.CreateDynamicBody(
                ...
                fixtures=b2FixtureDef(shape=b2PolygonShape(box=(0.8 * LEG_W / 2, LEG_H / 2)),
...
class MultiWalkerEnv(AbstractMAEnv, EzPickle):
    ...
    def __init__(self, n_walkers=2, position_noise=1e-3, angle_noise=1e-3, reward_mech='local',
                 forward_reward=1.0, fall_reward=-100.0, drop_reward=-100.0, ir_use_pos=False, terminate_on_fall=False,
                 one_hot=False):
    ...
    def setup(self):
        ...
        self.prev_pos = np.zeros(self.n_walkers)
        ...
        for walker in self.walkers:
            self.observation_space.append(walker.observation_space)
            self.action_space.append(walker.action_space)
            obs_dim += walker.observation_space.shape[0]
        self.share_observation_space = [spaces.Box(
            low=-np.inf, high=+np.inf, shape=(obs_dim,), dtype=np.float32) for _ in range(self.n_walkers)]
    ...
    def step(self, actions):
        ...
        rewards = []
        info_n = [{'individual_reward': 0., 'team_reward': 0.} for _ in range(self.n_walkers)]
        ...
        for i in range(self.n_walkers):
            ...
            idv_reward = (shaping - self.prev_shaping[i]) + (self.fall_reward * self.fallen_walkers[i])
            # rewards[i] = shaping - self.prev_shaping[i]
            # info_n[i]['individual_reward'] += (shaping - self.prev_shaping[i])
            if self.ir_use_pos:
                idv_reward += 5.0 * (x - self.prev_pos[i])
            self.prev_pos[i] = x
            self.prev_shaping[i] = shaping
            info_n[i]['individual_reward'] += idv_reward
            rewards.append([idv_reward])

        package_shaping = self.forward_reward * 130 * self.package.position.x / SCALE
        # rewards += (package_shaping - self.prev_package_shaping)
        team_reward = package_shaping - self.prev_package_shaping
        ...
        for i in range(self.n_walkers):
            info_n[i]['team_reward'] = team_reward

        if self.reward_mech == 'rs':
            rewards = [[team_reward + idv_r[0]] for idv_r in rewards]
        elif self.reward_mech == 'team':
            rewards = [[team_reward] for _ in range(self.n_walkers)]
        else:
            rewards = rewards

        return obs, rewards, [done] * self.n_walkers, info_n
    ...
    def _generate_package(self):
        ...
        self.package = self.world.CreateDynamicBody(
            position=(init_x, init_y),
            fixtures=b2FixtureDef(
                shape=b2PolygonShape(vertices=[(x * self.package_scale / SCALE, y / SCALE)
    ...
    def _generate_terrain(self, hardcore):
        ...
        for i in range(self.terrain_length):
            ...
            if state == GRASS and not oneshot:
                ...
                t = self.world.CreateStaticBody(fixtures=b2FixtureDef(
                    shape=b2PolygonShape(vertices=poly), friction=FRICTION))
                t.color1, t.color2 = (1, 1, 1), (0.6, 0.6, 0.6)
                self.terrain.append(t)
                t = self.world.CreateStaticBody(fixtures=b2FixtureDef(shape=b2PolygonShape(
        ...
        for i in range(self.terrain_length - 1):
            ...
            t = self.world.CreateStaticBody(fixtures=b2FixtureDef(
                shape=b2EdgeShape(vertices=poly),
                friction=FRICTION,
                categoryBits=0x0001,))
