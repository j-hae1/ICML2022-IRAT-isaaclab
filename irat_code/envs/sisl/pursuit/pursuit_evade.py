...
class PursuitEvade(AbstractMAEnv, EzPickle):
    ...
    def __init__(self, x_size=16, y_size=16, **kwargs):
        EzPickle.__init__(self, x_size, y_size, **kwargs)
        """
        ...
        # self.xs = xs
        # self.ys = ys
        tmp_seed = kwargs.pop('seed', 1)
        self.seed(tmp_seed)
        self.xs = x_size
        self.ys = y_size
        self.map_matrix = rectangle_map(self.xs, self.ys)

        self._reward_mech = kwargs.pop('reward_mech', 'global')
        self.max_cycles = kwargs.pop('max_cycles', 200)
        self.cur_step = 0
        ...
        self.pursuer_layer = kwargs.pop('ally_layer', AgentLayer(self.xs, self.ys, self.pursuers))
        self.evader_layer = kwargs.pop('opponent_layer', AgentLayer(self.xs, self.ys, self.evaders))
        ...
        self.current_agent_layer = np.zeros((self.xs, self.ys), dtype=np.int32)
        ...
        self.team_dec = kwargs.pop("team_dec", False)
        ...
        if self.train_pursuit:
            self.low = np.array([0.0 for _ in range(3 * self.obs_range**2)])
            self.high = np.array([1.0 for _ in range(3 * self.obs_range**2)])
            if self.include_id:
                self.low = np.append(self.low, 0.0)
                self.high = np.append(self.high, 1.0)
            action_spc = spaces.Discrete(n_act_purs)
            if self.flatten:
                observation_spc = spaces.Box(self.low, self.high)
            else:
                observation_spc = spaces.Box(low=-np.inf, high=np.inf,
                                             shape=(self.obs_range, self.obs_range, 4))
            for _ in range(self.n_pursuers):
                self.observation_space.append(observation_spc)
                self.action_space.append(action_spc)
                obs_dim += np.product(observation_spc.shape)
            if self.flatten:
                tmp_low, tmp_high = 0.0, 1.0
            else:
                tmp_low, tmp_high = -np.inf, np.inf
            self.share_observation_space = [
                spaces.Box(
                    low=tmp_low,
                    high=tmp_high,
                    shape=(obs_dim,), dtype=np.float32) for _ in range(self.n_pursuers)
            ]
    ...
    def step(self, actions):
        ...
        # if self.team_dec:
        #     team_rewards = self.term_pursuit * pursuers_who_remove
        # else:
        #     team_rewards = np.zeros(self.n_pursuers) + self.term_pursuit * ev_remove
        # urgency reward to speed up catching
        # rewards += self.urgency_reward

        if self.team_dec:
            idv_rewards += self.term_pursuit * pursuers_who_remove
        idv_rewards += self.urgency_reward

        team_rewards = np.zeros(self.n_pursuers) + self.term_pursuit * ev_remove

        if self.reward_mech == "rs":
            rewards = [[team_rewards[i] + idv_rewards[i]] for i in range(self.n_pursuers)]
        elif self.reward_mech == "team":
            rewards = [[team_rewards[i]] for i in range(self.n_pursuers)]
        else:
            rewards = [[ir] for ir in idv_rewards]

        info_n = [{'individual_reward': 0., 'team_reward': 0.} for _ in range(self.n_pursuers)]
        for i in range(self.n_pursuers):
            info_n[i]["individual_reward"] = idv_rewards[i]
            info_n[i]["team_reward"] = team_rewards[i]
        ...
        return obslist, rewards, [done] * self.n_pursuers, info_n
    ...
    def render(self, mode="rgb_array", plt_delay=0.2):
