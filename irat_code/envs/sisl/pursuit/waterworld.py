...
class MAWaterWorld(AbstractMAEnv, EzPickle):
    ...
    def __init__(self, n_pursuers, n_evaders, n_coop=2, n_poison=10, radius=0.015,
                 ...
                 reward_mech='local', addid=True, speed_features=True, max_cycles=1000, idv_use_caught_food=False,
                 **kwargs):
        EzPickle.__init__(self, n_pursuers, n_evaders, n_coop, n_poison, radius, obstacle_radius,
                          ...
                          control_penalty, reward_mech, addid, speed_features, max_cycles, **kwargs)
        self.n_pursuers = n_pursuers
        self.n_evaders = n_evaders
        self.n_coop = n_coop
        self.n_poison = n_poison
        self.obstacle_radius = obstacle_radius
        self.obstacle_loc = np.array(obstacle_loc)
        ...
        self.idv_use_caught_food = idv_use_caught_food
        ...
        for pursuer in self._pursuers:
            self.observation_space.append(pursuer.observation_space)
            self.action_space.append(pursuer.action_space)
            obs_dim += pursuer.observation_space.shape[0]
        self.share_observation_space = [spaces.Box(
            low=-10, high=10, shape=(obs_dim,), dtype=np.float32) for _ in range(self.n_pursuers)]
    ...
    def reset(self):
        ...
        obs = self.step(np.zeros((self.n_pursuers, 2)))[0]
        # print(len(obs))
    ...
    def step(self, action_Np2):
        ...
        idv_rewards = np.zeros((self.n_pursuers,))
        ...
        idv_rewards += self.control_penalty * (actions_Np_2**2).sum(axis=1)
        ...
        idv_rewards[which_pursuer_caught_po] += self.poison_reward
        idv_rewards[which_pursuer_encounterd_ev] += self.encounter_reward
        if self.idv_use_caught_food:
            idv_rewards[which_pursuer_caught_ev] += self.food_reward

        team_rewards = len(ev_caught) * self.food_reward
        if self.reward_mech == "rs":
            rewards = [[team_rewards + ir] for ir in idv_rewards]
        elif self.reward_mech == "team":
            rewards = [[team_rewards] for _ in range(self.n_pursuers)]
        else:
            rewards = [[ir] for ir in idv_rewards]
        for i in range(self.n_pursuers):
            info_n[i]["individual_reward"] = idv_rewards[i]
            info_n[i]["team_reward"] = team_rewards
