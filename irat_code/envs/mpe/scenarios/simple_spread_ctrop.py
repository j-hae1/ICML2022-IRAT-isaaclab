...
class Scenario(BaseScenario):
    def make_world(self, args):
        ...
        world.mode = getattr(args, "game_mode", "easy")
        world.cur_steps = 0
    ...
    def individual_reward(self, agent, world):
        ...
        if world.mode == "easy":
            ...
            for ld in world.landmarks:
                if not ld.visited:
                    dists.append(np.sqrt(np.sum(np.square(agent.state.p_pos - ld.state.p_pos))))
            if len(dists) > 0:
                rew += 1.0 / (1.0 + min(dists))
        else:
            for ld in world.landmarks:
                dist = np.sqrt(np.sum(np.square(ld.state.p_pos - agent.state.p_pos)))
                if not ld.visited and dist < (ld.size + agent.size):
                    rew += 5.0
