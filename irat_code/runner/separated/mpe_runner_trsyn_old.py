...
from irat_code.runner.separated.base_runner_trsyn_old import Runner
import imageio
    ...
    def eval(self, total_num_steps, title):
        ...
        for agent_id in range(self.num_agents):
            ...
            if catch_infos is not None:
                tinfos["eval_idv_catch_total_num"] = np.sum(catch_infos[:, :, agent_id, 0])
                print("eval idv catch total num of agent%i: " % agent_id + str(np.sum(catch_infos[:, :, agent_id, 0])))
                tinfos["eval_team_catch_total_num"] = np.sum(catch_infos[:, :, agent_id, 1])
                print("eval team catch total num: " + str(np.sum(catch_infos[:, :, agent_id, 1])))
