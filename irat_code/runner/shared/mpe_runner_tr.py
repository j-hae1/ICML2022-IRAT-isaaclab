...
class MPERunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            if self.use_linear_lr_decay:
                self.team_trainer.policy.lr_decay(episode, episodes)
                self.idv_trainer.policy.lr_decay(episode, episodes)
            ...
            for step in range(self.episode_length):
                ...
                data = obs, rewards, dones, infos, values, actions, action_log_probs, team_log_probs, rnn_states, \
                       rnn_states_critic, team_act_dists
                self.idv_insert(data)

                # insert data into team buffer
                data = obs, rewards, dones, infos, team_values, team_actions, team_log_probs, action_log_probs, \
                       team_rnn, team_rnn_critic, act_dists
    ...
    def warmup(self):
        ...
        self.team_buffer.share_obs[0] = share_obs.copy()
        self.team_buffer.obs[0] = obs.copy()

        self.idv_buffer.share_obs[0] = idv_share_obs.copy()
        self.idv_buffer.obs[0] = obs.copy()

    @torch.no_grad()
    def idv_collect(self, step):
        self.idv_trainer.prep_rollout()
        value, action, action_log_prob, rnn_states, rnn_states_critic, act_dist \
            = self.idv_trainer.policy.get_actions(np.concatenate(self.idv_buffer.share_obs[step]),
                                                  np.concatenate(self.idv_buffer.obs[step]),
                                                  np.concatenate(self.idv_buffer.rnn_states[step]),
                                                  np.concatenate(self.idv_buffer.rnn_states_critic[step]),
                                                  np.concatenate(self.idv_buffer.masks[step]))
        ...
        return values, actions, action_log_probs, rnn_states, rnn_states_critic, actions_env, act_dist
