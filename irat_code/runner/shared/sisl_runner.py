...
class SISLRunner(Runner):
            ...
            if episode % self.log_interval == 0:
                ...
                for agent_id in range(self.num_agents):
                    agent_infos[agent_id].update(
                        {'average_step_individual_rewards': np.mean(idv_rewards[:, :, agent_id])})
                    agent_infos[agent_id].update(
                        {"average_episode_team_rewards": np.mean(np.sum(team_rewards[:, :, agent_id], axis=0))})
    ...
    def collect(self, step):
        ...
        value, action, action_log_prob, rnn_states, rnn_states_critic, _ \
        ...
        if self.envs.action_space[0].__class__.__name__ == 'Box' and self.all_args.action_use_clip:
            actions = np.clip(actions, self.envs.action_space[0].low[0], self.envs.action_space[0].high[0])
            value, action_log_prob, _, _ = self.trainer.policy.evaluate_actions(
    ...
    def eval(self, total_num_steps, title):
        ...
        for eval_step in range(self.episode_length):
            ...
            if self.envs.action_space[0].__class__.__name__ == 'Box' and self.all_args.action_use_clip:
                eval_actions = np.clip(eval_actions, self.envs.action_space[0].low[0], self.envs.action_space[0].high[0])

            if self.envs.action_space[0].__class__.__name__ == 'Box':
                eval_actions_env = eval_actions
            elif self.envs.action_space[0].__class__.__name__ == 'Discrete':
                eval_actions_env = eval_actions.squeeze(2)
            else:
                raise NotImplementedError
